...
{
	if (c == '\r' && !lastCharWasCR && headerName.empty())
	{
		lastCharWasCR = true;
		// Do nothing, expect line feed next
	}
	else if (c == '\n' && lastCharWasCR && headerName.empty())
	{
...
void	HeaderFieldStateMachine::handleStateHeaderOWS(char c)
{
	...
	else if (c == '\r' && !lastCharWasCR)
	{
		lastCharWasCR = true;
		// Do nothing, expect line feed next
	}
	else if (c == '\n' && lastCharWasCR && !headerValue.empty())
	{
		// End of line, transition states
		lastCharWasCR = false;
...
{
	if (c == '\r' && !lastCharWasCR)
	{
		lastCharWasCR = true;
		// Do nothing, expect line feed next
	}
	else if (c == '\n' && lastCharWasCR && !headerValue.empty()) //TODO: handle duplicate header fields
	{
		// End of line, transition states
		lastCharWasCR = false;
